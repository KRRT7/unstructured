...
def group_elements_by_parent_id(
    ...
    """
    groups: dict[Optional[str], list["Element"]] = {}
    last_parent_id: Optional[str] = None

    # Local bindings to reduce global lookups in the hot loop
    _getattr = getattr
    _obj_getattr = object.__getattribute__

    for element in elements:
        # Access metadata exactly as original code: if the element has no 'metadata'
        # attribute, this should raise AttributeError (preserve original semantics).
        metadata = _obj_getattr(element, "metadata")
        parent_id = _getattr(metadata, "parent_id", None)
        ...
        lst = groups.get(parent_id)
        if lst is None:
            groups[parent_id] = [element]
        else:
            lst.append(element)
